...
ERRORS=()
#RETURN_CODE=0 ## 0 - not used

### Run Some Checks ###
## Check For Last Error ##
if [ $"LAST_ERRNO" != 0 ]   #1- LAST_ERRNO to "LAST_ERRNO"
then
...
if [ "$SECONDS_BEHIND_MASTER" == 'NULL' ] ### 2- SECONDS_BEHIND_MASTER to "SECONDS_BEHIND_MASTER"
then
    ERRORS=("${ERRORS[@]}" "The Slave is reporting 'NULL' (Seconds_Behind_Master)")
elif [ "$SECONDS_BEHIND_MASTER" -gt 60 ]  # 3- SECONDS_BEHIND_MASTER to "SECONDS_BEHIND_MASTER"
then
